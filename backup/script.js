...
const UIManager = {
    ...
    updateUserInterface() {
        if (AuthManager.isAuthenticated()) {
            ...
            // Update navigation - add Dashboard link
            const nav = document.querySelector('nav ul');
            if (nav && !document.querySelector('nav a[href="dashboard.html"]')) {
                // Check if we're on index.html (has anchor links like #home)
                const firstLink = nav.querySelector('a');
                if (firstLink && firstLink.getAttribute('href')?.startsWith('#')) {
                    // Add Dashboard link after Beranda
                    const dashboardLi = document.createElement('li');
                    dashboardLi.innerHTML = '<a href="dashboard.html">Dashboard</a>';
                    const berandaLi = nav.querySelector('li:first-child');
                    if (berandaLi) {
                        berandaLi.after(dashboardLi);
                    }
                }
            }
            ...
            if (loginBtn) {
                loginBtn.outerHTML = `
                    <div class="user-menu-logged">
                        <a href="dashboard.html" class="btn btn-secondary" style="margin-right: 8px;">
